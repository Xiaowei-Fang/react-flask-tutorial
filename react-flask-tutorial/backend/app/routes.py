...
from .ml_services import DigitRecognizerService

api_bp = Blueprint('api', __name__)
chart_service = ChartDataService()
digit_recognizer_service = DigitRecognizerService()
...
@api_bp.route('/ml/predict-digit', methods=['POST'])
@jwt_required() # 要求用户登录后才能使用这个功能
def predict_digit():
    # 检查是否有文件被上传
    ...
    if file:
        # 调用服务进行预测
        result = digit_recognizer_service.predict(file)
        if "error" in result:
            return jsonify(result), 500 # 如果预测出错，返回服务器错误
